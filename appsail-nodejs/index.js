...
app.get('/zoho-data', async (req, res) => {
  try {
    const data = await fetchDataFromZohoSheets(process.env.ZOHO_RESOURCE_ID, process.env.ZOHO_WORKSHEET_NAME2);
    res.json(data); 
  } catch (error) {
...
app.post('/customer-detail', async (req, res) => {
  try { const { Name, Age, Phone,row_index,
   } = req.body;
   const updates = { 
     'Name': Name,
      'Age': Age, 
...
async function updateZohoSheet(updates){
  try {
      ...
      const data = {
          worksheet_name: process.env.ZOHO_WORKSHEET_NAME2,
          method: 'range.content.get',
          start_row: 1,
          start_column: 1,
          end_row: 5000, // Adjust the row range as needed
